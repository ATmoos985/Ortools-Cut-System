...
三次独立运行（含无记忆的 480s 首跑）**6 次交换序列逐字节一致**，
确定性得到跨运行验证；记忆消除约 42% 重复扫描。25 组在 480s 预算内
仍未完成束≤6 的最终验证轮（估算完整一轮约需 5,600 束 × ~70ms），
故 **25 仍是预算下界，不是已证明的邻域不动点**。
...
1. 实现 incumbent 持久化/续跑（从列签名重建 GroupColumn），
   突破单次运行预算上限，完成束≤6 的不动点证明或找到第 7 次交换；
2. 束枚举优先级改为"围绕上次成功交换的列邻域"，加速最终验证轮；
3. seeder 支持集 fallback（§6.7 生产待办）；
