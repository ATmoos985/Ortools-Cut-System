...
### 6.5 证否束记忆与复现验证（2026-07-26 晚，commit 9870ddc）

给 pricer 增加"已证否束记忆"：只记忆**枚举未截断且 MIP 返回
INFEASIBLE（证明级）**的束；记忆连同当时被 family 过滤挡掉的家族
集合一起存储，任一被挡家族离开保留集时记忆自动失效——不引入任何
静默漏扫。超时、截断一律不记忆。
...
| 预算 | 结果 | 扫描 | 记忆跳过 | 预算耗尽 |
|---|---|---:|---:|---|
| 360s | 25/1/0/169/36,870 | 5,697 | 4,208 | 是 |
| 480s | 25/1/0/169/36,870 | 6,991 | 5,174 | 是 |

四次独立运行（含无记忆的 480s 首跑与落盘跑）**6 次交换序列逐字节
一致**，确定性得到跨运行验证；记忆消除约 42% 重复扫描。

### 6.5.1 落盘/续跑与 25 组邻域证否（commit 2d018e0）

实现 incumbent 落盘（最终列签名写入
`target/djx188-residual-bundle-incumbent.txt`）与签名续跑
（`-Dcutting.test.residualBundle.resume=<file>`，`parseColumn`
自带 round-trip 校验）。续跑加载 <1 秒，跳过整个自主求解，
不再受 seeder 彩票影响。

对同一 25 组解的分腿验证（每腿 540s 纯扫描，全部**零交换**）：

| 腿 | 束尺寸范围 | 覆盖 |
|---|---|---|
| r2 | 2→4 | 尺寸2 全部300束、尺寸3 全部2300束、尺寸4 前1000束 |
| r3 | 5→6 | 尺寸5 前1000束、尺寸6 前749束 |
| r4 | 6 | 尺寸6 前794束（与 r3 部分重叠） |

结论：25 组在"尺寸2-3 **完整**邻域 + 尺寸4/5/6 重叠度排序前
1000/1000/794 束"内已实证零改进（累计约 5,400 束证否，零截断）。
严格的完整不动点证明还差：尺寸4 未排名的 11,650 束、尺寸5/6 的
长尾（53,130/177,100 总量）。尺寸6 单束验证成本约 0.7 秒。

### 6.6 T42 跨数据集复验

`-Dcutting.test.residualBundle.t42=true`：基线 10 组（45车/10,820mm/
奇1/单0），束≤6、120s 预算扫描 807/837 个束，零截断零交换，
全守恒保持。1/1 通过。说明：(a) 机制跨数据集工作正常、无假交换；
(b) T42 的 10 组在束≤6 邻域内接近局部最优（剩 30 束未扫）。

### 6.7 新发现：seeder 支持集墙钟彩票（当日实测）

当晚机器变慢后自主路径连续失败，诊断（先打印后断言的教训已回写
进实验测试）定位到 `OrderGroupColumnIncumbentSeeder`：

- 支持花型 MIP 是墙钟截断的——预算/机速不同返回不同支持集
  （观测到 26 花型成功 vs 23 花型失败）；
- 23 花型支持集上组列 RMP 定价收敛（`NO_NEGATIVE_COLUMN_FOUND`）
  后整数主问题 `INFEASIBLE`——**时间截断的支持集可能整数不可行**，
  是定价文档 §15.1"LP可行≠整数RMP可行"在 seeder 内部的复现；
- seeder 组列阶段的预算来自 `pricingTimeLimitMs`（内层 `solveCore`
  完整继承 Options），与 `patternSeedTimeLimitMs` 无关——只调后者
  救不了该阶段。

harness 侧缓解（已实现）：`pricingMs` 可 `-D` 提高（慢机器建议
20000）+ 实验测试内 seed 预算 {10s,20s,40s} 确定性重摇。生产侧
待办：seeder 需要支持集不可行时的 fallback/修复回路（换一个支持
集重试，而不是整体失败）。

### 6.8 下一步

1. ~~incumbent 持久化/续跑~~（已完成，见 §6.5.1）；补充跨腿扫描
   偏移或记忆持久化，以覆盖尺寸 4-6 的长尾束；
2. 若长尾仍零改进，通往 24 及以下需要升级机制：更大束尺寸
   （31↔29 见证交换的连通规模是 17↔15）、或残量内的列生成式
   替换（不受枚举上限约束）；
3. 束枚举优先级改为"围绕上次成功交换的列邻域"，加速验证轮；
4. seeder 支持集 fallback（§6.7 生产待办）；
5. 达到稳定天花板后再评估生产接入路径（当前仍为研究原型，
   生产门禁保持关闭）。anytime 性质已具备：每次交换后即完整可行解，
   90 秒 28 组 / 数分钟 25 组。
